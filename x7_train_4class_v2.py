...
def to_8channel_tensor(frames: np.ndarray) -> torch.Tensor:
    # frames shape: (64, 2, 2, 96) complex64 -> (Time, TX, RX, Bins)
    tx0_rx0 = frames[:, 0, 0, :]
    tx0_rx1 = frames[:, 0, 1, :]
    tx1_rx0 = frames[:, 1, 0, :]
    tx1_rx1 = frames[:, 1, 1, :]
    
    # Stack into 8 channels: (8, 64, 96)
    stacked = np.stack([
...
class X7GestureDatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # True gesture: Peak should be near the end of the window (Index 45)
        # This teaches the model to recognize the gesture as it finishes!
        start = peak_frame - 45
        if self.augment:
            start += np.random.randint(-7, 8) # Peak ranges from 38 to 52
...
def run(args):
    ...
    try:
        ...
        for ep in range(1, args.epochs + 1):
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                torch.save(model.state_dict(), os.path.join(args.out, "best_model_4class_v2.pth"))
