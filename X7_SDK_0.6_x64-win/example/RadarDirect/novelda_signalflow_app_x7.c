...
{
    novelda_product_error_t result = radar_direct_set_chip_count(rd, 1);
    if( result != PRODUCT_ERROR_SUCCESS ) {
        printf("Failed to set chip count: %" PRId32 "\n", result);
        return result;
    }
    ...
    trx_config_t trx_config = {
        .tx_channel_sequence = (const uint16_t[]) { 0, 1 },
        .tx_channel_sequence_length = 2,
        .rx_mask_sequence = (const uint16_t[]) { 0b11, 0b11 },
        .rx_mask_sequence_length = 2
    };
    result = radar_direct_configure_trx(rd, &trx_config);
    if( result != PRODUCT_ERROR_SUCCESS ) {
        printf("Failed to configure TRX: %" PRId32 "\n", result);
        return result;
    }

    x7_chip_config_t chip_config = {
        .fps = 250.0f,
        .pulse_period = 6,
        .mframes_per_pulse = 6,
        .pulses_per_iteration = 35,
        .iterations_per_frame = 620,
        .tx_power = 2,
        .interleaved_frames = 0
    };

    result = radar_direct_set_x7_chip_config(rd, &chip_config);
    if( result != PRODUCT_ERROR_SUCCESS ) {
        printf("Failed to set chip config: %" PRId32 "\n", result);
        return result;
    }
