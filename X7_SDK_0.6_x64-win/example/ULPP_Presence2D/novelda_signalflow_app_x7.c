...
static signalflow_error_t ulpp_process_output(void *host_context, uint32_t node_name, const uint8_t *buffer, size_t buffer_size)
{
    ...
    if( data.datatype != SF_DATATYPE_INT16 ) {
    ...
    if( data.array_element_count > 0 ) {
        printf("Presence: %01d, Position (cm): (%4d,%4d), Confidence: %03d",
               ((int16_t *)data.array)[0],
               ((int16_t *)data.array)[1],
               ((int16_t *)data.array)[2],
               ((int16_t *)data.array)[3]);
    }
    ...
    result = ulpp_read_human_detection(ulpp, buffer, buffer_size, &data);
    if( result != SFERR_SUCCESS ) {
        printf("Failed to read human detection: %" PRId32 "\n", result);
        return result;
    }

    if( data.array_element_count > 0 ) {
        printf("  Human detection 2D raw data:\n");
        for( size_t i = 0; i < data.array_element_count; i++ ) {
            if( i % data.shape[1] == 0 ) {
                printf("    [ ");
            }
            // printf converts floats to double always, so cast it to avoid compiler warning
            printf("%.2f ", (double)((float *)data.array)[i]);
            if( (i + 1) % data.shape[1] == 0 ) {
                printf("]\n");
            }
        }
    }
...
static novelda_product_error_t ulpp_configure(ulpp_context_t *ulpp, int argc, char *argv[])
{
    ...
    // If you have a X7T02 module you need to set this to true
    const int has_x7t02_module = 0;
    if( has_x7t02_module ) {
        result = ulpp_set_x7t02_module(ulpp);
        if( result != PRODUCT_ERROR_SUCCESS ) {
            printf("Failed to set X7T02 module: %" PRId32 "\n", result);
            return result;
        }
    }
